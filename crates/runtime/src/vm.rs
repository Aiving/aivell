use crate::{
    ...
    DefaultStderr, DefaultStdin, DefaultStdout, KAsyncNativeFunction, KCaptureFunction, KFunction,
    Ptr, Result,
};
use bytecode::{Chunk, Instruction, InstructionReader, Loader};
use instant::Instant;
use parser::{ConstantIndex, MetaKeyId, StringAlignment, StringFormatOptions};
...
impl KotoVm {
    ...
    fn call_external(&mut self, call_info: &CallInfo, callable: ExternalCallable) -> Result<()> {
        ...
        let result = match callable {
            ExternalCallable::AsyncFunction(f) => {
                let handle = Handle::current();

                handle.block_on((f.function)(&mut call_context))
            }
    ...
    fn call_callable(
        ...
        match callable {
            ...
            AsyncNativeFunction(f) => self.call_external(info, ExternalCallable::AsyncFunction(f)),
    ...
    fn run_debug(&mut self, register: u8, expression_constant: ConstantIndex) -> Result<()> {
        ...
        let prefix = match (
            ...
            (Some(span), Some(path)) => format!("[{}: {}] ", path.display(), span.line + 1),
            (Some(span), None) => format!("[{}] ", span.line + 1),
...
enum ExternalCallable {
    AsyncFunction(KAsyncNativeFunction),
