...
use crate::prelude::*;
use crate::Result;
use bytecode::CompilerSettings;
use deriver::{KotoCopy, KotoType};
use memory::Ptr;
...
pub struct Chunk(Ptr<bytecode::Chunk>);

impl Chunk {
    fn inner(&self) -> Ptr<bytecode::Chunk> {
        Ptr::clone(&self.0)
    }
}
...
impl From<Ptr<bytecode::Chunk>> for Chunk {
    fn from(inner: Ptr<bytecode::Chunk>) -> Self {
        Self(inner)
    }
}
