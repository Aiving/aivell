...
type BoxFuture<T> = Pin<Box<dyn Future<Output = T> + KotoSend + KotoSync + 'static>>;
...
pub trait AsyncKotoFunction:
    Fn(&mut CallContext) -> BoxFuture<Result<KValue>> + KotoSend + KotoSync + 'static
{
}

impl<T> AsyncKotoFunction for T where
    T: Fn(&mut CallContext) -> BoxFuture<Result<KValue>> + KotoSend + KotoSync + 'static
{
}
...
///
/// See [KValue::NativeFunction]
pub struct KAsyncNativeFunction {
    /// The function implementation that should be called when calling the external function
    //
    // Disable a clippy false positive, see https://github.com/rust-lang/rust-clippy/issues/9299
    // The type signature can't be simplified without stabilized trait aliases,
    // see https://github.com/rust-lang/rust/issues/55628
    #[allow(clippy::type_complexity)]
    pub function: Ptr<dyn AsyncKotoFunction>,
}
...
impl KAsyncNativeFunction {
    /// Creates a new external function
    pub fn new(function: impl AsyncKotoFunction) -> Self {
        Self {
            function: make_ptr!(function),
        }
    }
...
impl Clone for KAsyncNativeFunction {
    fn clone(&self) -> Self {
        Self {
            function: self.function.clone(),
        }
...
impl fmt::Debug for KAsyncNativeFunction {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "external function: {:?}", Ptr::address(&self.function))
    }
}
...
impl Hash for KAsyncNativeFunction {
    fn hash<H: Hasher>(&self, state: &mut H) {
        Ptr::address(&self.function).hash(state)
    }
}
