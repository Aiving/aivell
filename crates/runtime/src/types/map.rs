...
use std::{
    any::Any, hash::{BuildHasherDefault, Hash}, ops::{Deref, DerefMut, RangeBounds}
};

use super::native_function::{AsyncKotoFunction, KAsyncNativeFunction};
...
#[derive(Clone, Default)]
pub struct KMap {
    state: Ptr<Option<Box<dyn Any + Send + Sync>>>,
...
impl KMap {
    ...
    /// Adds a function to the KMap's data map
    pub fn add_async_fn(&self, id: &str, f: impl AsyncKotoFunction) {
        self.insert(id, KValue::AsyncNativeFunction(KAsyncNativeFunction::new(f)));
    }
