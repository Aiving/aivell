...
pub fn format_source_excerpt(
    ...
    Report::build(ReportKind::Error, path.as_str(), 0)
        .with_message(message)
        .with_label(Label::new((path.as_str(), span.start..span.end)).with_color(Color::Red))
        .finish()
        .write((path.as_str(), Source::from(source)), &mut data)
        .unwrap();
