...
use parser::{
    Ast, AstBinaryOp, AstFor, AstIf, AstIndex, AstNode, AstUnaryOp, AstVec, ChainNode, ConstantIndex, Function, MatchArm, MetaKeyId, Node, StringContents, StringFormatOptions, StringNode, Type, UseItem
};
use shared::Span;
...
type Result<T> = std::result::Result<T, Box<CompilerError>>;
...
impl Compiler {
        ...
        let result = match &node.node {
            ...
            Node::Function(f) => {
                if let Some(name) = f.name {
                    self.compile_named_function(name, f, false, ctx)?
                } else {
                    self.compile_function(f, ctx)?
                }
            }
            Node::Use { from, items } => self.compile_import(from, items, ctx)?,
            ...
            Node::Break => match self.frame().current_loop() {
                Some(loop_info) => {
                    let loop_result_register = loop_info.result_register;

                    if let Some(loop_result_register) = loop_result_register {
        ...
        match &type_node.node {
            Node::Type(type_index) => {
                if self.settings.enable_type_checks {
                    ...
                    if let Type::Named(type_index) = type_index {
    ...
    fn compile_named_function(
        ...
        let value_result =
            self.compile_function(function, ctx.with_register(value_result_register))?;
    ...
    fn compile_function(
        ...
        let result = if let Some(name) = function.name {
            match self.local_register_for_assign_target(name, ctx)? {
                Some(local) => CompileNodeOutput::with_assigned(local),
                None => CompileNodeOutput::with_temporary(self.push_register()?),
            }
        } else {
            self.assign_result_register(ctx)?
        };
        ...
        if let Some(result_register) = result.register {
            ...
            let flags_byte = FunctionFlags {
                ...
                generator: false, /*  function.is_generator */
            ...
            let allow_implicit_return = true/* !function.is_generator */;
            ...
            self.compile_frame(
                FrameParameters {
                    ...
                    is_generator: false, /* function.is_generator */
                },
                ctx,
            )?;
    ...
    fn compile_chain(
        ...
        if piped_arg_register.is_some() {
            let piped_call_args = AstVec::new();
    ...
    fn make_error(&self, error: impl Into<ErrorKind>) -> Box<CompilerError> {
        Box::new(CompilerError {
            error: error.into(),
            span: self.span(),
        })
    }
