...
use memory::Ptr;
use parser::{format_source_excerpt, Parser};
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if let Some(source) = &self.source {
            write!(
                f,
                "{}",
                format_source_excerpt(&source.contents, self.error.to_string(), &source.span, source.path.as_deref())
            )?;
        } else {
        ...
        match Parser::parse(script) {
            Ok(ast) => {
                let (bytes, mut debug_info) = match Compiler::compile(&ast, settings) {
                    ...
                    Err(e) => {
                        return Err(LoaderError::from_compiler_error(*e, script, script_path))
                    }
                };
